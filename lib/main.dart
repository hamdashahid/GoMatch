          ...
          if (snapshot.hasData) {
            ...
            return FutureBuilder<bool>(
              ...
              builder: (context, roleSnapshot) {
                ...
                return const Center(child: Text('Error determining user role.'));
              },
            );
          } else {
...
Future<bool> _checkUserRole(String? email) async {
  ...
  final driverCollection = FirebaseFirestore.instance.collection('driver_profile');
  final passengerCollection = FirebaseFirestore.instance.collection('passenger_profile');

  // Check if email exists in 'driver_profile'
  final driverSnapshot = await driverCollection.where('email', isEqualTo: email).get();
  if (driverSnapshot.docs.isNotEmpty) {
    return true; // User is a driver
  }

  // Check if email exists in 'passenger_profile'
  final passengerSnapshot = await passengerCollection.where('email', isEqualTo: email).get();
